...
if let Some(e) = data.entry(path, "new text") {
    e.insert(original_offset)?;
}

// Replace a range given in original-source coordinates:
if let Some(e) = data.entry(path, "replacement") {
    e.replace(original_start..original_end)?;
}
```

`entry()` returns `None` when the path is not part of the project's own sources (e.g. a library dependency). Both `insert` and `replace` return `eyre::Result<EditInfo>` with the position of the edit in the expanded source:

```rust
if let Some(e) = data.entry(path, "new text") {
    let info = e.insert(original_offset)?;
...
#### `OriginalOffset` span helpers

Converting a Solar HIR span position to an `OriginalOffset` requires accounting for the file's start position within the source map. Several helpers on `OriginalOffset` handle this correctly and return `eyre::Result` rather than performing unchecked arithmetic:

```rust
use reforge::OriginalOffset;

// Low-level: any BytePos relative to its source file.
let offset = OriginalOffset::from_solar_pos(&source.file, some_span.lo())?;

// Typed helpers that take the HIR item directly, preventing accidental use of the wrong span:
let close_brace = OriginalOffset::end_of_contract(&source.file, contract)?; // span.hi() - 1
let contract_start = OriginalOffset::contract_offset(&source.file, contract)?; // span.lo()
let func_start = OriginalOffset::func_offset(&source.file, func)?;            // span.lo()
```

`source.file` is the `SourceFile` obtained from `ctx.sources.get(item.source)`.
