...
pub fn correct_fmt_msg(macros: &MacroRules, e: &mut SolcError) {
    ...
    let attribution = macros.with_offset_adjustments(|adjustments| {
        adjustments
...
fn remap_fmt_msg(
    ...
    let remapped = macros.with_offset_adjustments(|adjustments| {
        adjustments.get_original_line(source, primary.line_num())
    });
...
fn remap_framed_line(macros: &MacroRules, source: &Path, line: &str) -> String {
    ...
    let remapped = macros.with_offset_adjustments(|adjustments| {
        adjustments.get_original_line(source, line_num as isize)
    });
